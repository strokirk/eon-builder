...
export function skillValueToDice(value: number): string {
  if (value < 0) return "-"
  if (SKILL_UNITS_TO_DICE[value] !== undefined) return SKILL_UNITS_TO_DICE[value]
...
export function getChunkAssignments(state: Eon5CharState): Map<number, AttributeName> {
...
export function calculateOverflow(skillValue: number, maxValue: number): number {
...
export function validateAttributes(state: Eon5CharState): ValidationError[] {
  ...
  if (!state.distributionModel) {
    errors.push({
      ...
      message: "Välj en fördelningsmodell",
      severity: "error",
    })
    return errors
  }
  ...
  const chunks = getChunks(state.distributionModel)
  const assignedCount = ATTRIBUTES.filter((a) => state.attributes[a].assignedChunk !== null).length

  // For "Fria poäng", validate total points
  if (isFreePoints) {
    const totalPoints = getTotalAttributePoints(state.extraAttributePoints)
    const usedPoints = ATTRIBUTES.reduce(
      (sum, attr) => sum + (state.attributes[attr].assignedChunk ?? 0),
      0,
    )
  ...
  // Check for duplicate chunk usage (only for preset models)
  if (!isFreePoints) {
...
export function validateSkills(state: Eon5CharState): ValidationError[] {
  ...
  if (wisdomAttr) {
    ...
    if (wisdomEntry.baseValueCount > 0) {
      ...
      const requiredCount = Math.min(wisdomEntry.baseValueCount, TOTAL_KNOWLEDGE_SKILLS)
      if (
...
export function getSkillGroupForUnitType(unitType: string): string | undefined {
...
export function createInitialState(): Eon5CharState {
  ...
  for (const attr of ATTRIBUTES) {
    attributes[attr] = { base: 8, modifiers: 0, assignedChunk: null }
  }
