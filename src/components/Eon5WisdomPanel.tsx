  ...
  return (
    <div className="space-y-3 p-3 border rounded bg-gray-50">
      <h4 className="text-sm font-medium">
        Visdom-panel (Visdom: {displayValue})
      </h4>
      ...
      <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-sm">
        ...
        <div>
          Ny kunskap-kostnad (erfarenhet):{" "}
          <strong>{entry.newKnowledgeCost}</strong>
        </div>
      </div>
      ...
      {entry.incompetentCount === TOTAL_KNOWLEDGE_SKILLS && (
        <p className="text-sm text-red-600">
          Samtliga kunskapsfärdigheter är Inkompetenta.
        </p>
      )}
      ...
      {entry.baseValueCount === TOTAL_KNOWLEDGE_SKILLS && (
        <p className="text-sm text-green-700">
          Samtliga kunskapsfärdigheter har Grundvärde 1.
        </p>
      )}
    </div>
  )
  ...
  return (
    <div className="space-y-1">
      <p className="text-sm font-medium">
        ...
        {remaining > 0 && (
          <span className="text-red-600 ml-1">({remaining} kvar)</span>
        )}
        {remaining === 0 && (
          <span className="text-green-600 ml-1">(klart)</span>
        )}
      </p>
  ...
  return (
    <div className="space-y-1">
      <p className="text-sm font-medium">
        ...
        {remaining > 0 && (
          <span className="text-amber-600 ml-1">({remaining} kvar)</span>
        )}
        {remaining === 0 && (
          <span className="text-green-600 ml-1">(klart)</span>
        )}
