...
export function Eon5Summary() {
  ...
  return (
    <div className="space-y-4">
      ...
      {wisdomEntry && (
        <div className="space-y-1 text-sm">
          <h4 className="font-medium fg-eon-red">
            Visdom ({wisdomFinal})
          </h4>
          <p>
            Ny kunskap-kostnad: <strong>{wisdomEntry.newKnowledgeCost}</strong>{" "}
            erfarenhetspoäng
          </p>
                    ...
                    <tr key={skill.name}>
                      ...
                      <td className="text-center">
                        {skill.status && (
                          ...
                          >
                            {
                              SKILL_STATUS_INFO[skill.status]
                                .name
                            }
                          </span>
                        )}
          ...
          {DESENSITIZATION_CATEGORIES.map((cat) => {
            ...
            return (
              <p key={cat}>
                ...
                {DESENSITIZATION_THRESHOLDS.filter((t) => value >= t).length >
                  0 && (
                  <span className="text-gray-500 ml-1">
                    (tröskel{" "}
                    {DESENSITIZATION_THRESHOLDS.filter((t) => value >= t).join(
                      ", ",
                    )}
                    )
                  </span>
